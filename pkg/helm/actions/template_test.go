...
func TestListReleaseWithNoValues(t *testing.T) {
...
`
	...
	rel, err := act.RenderManifests("test", chart, nil)
	if err != nil {
		t.Error(err)
	}

	assert.Equal(t, templateValue, rel, "")
}

func TestRenderManifestsWithValues(t *testing.T) {
	chart := "../testdata/influxdb-3.0.2.tgz"
	templateValue := `---
# Source: influxdb/templates/config.yaml
apiVersion: v1
...
`
	...
	act := NewHelmActions(actionConfig)
	js := []byte("{\"service\": { \"type\": \"NodePort\" }, \"persistence\": {\"size\": \"16Gi\"}}")
	var m map[string]interface{}
	json.Unmarshal(js, &m)

	txt, err := act.RenderManifests("test", chart, m)
	if err != nil {
		t.Error(err)
	}

	assert.Equal(t, templateValue, txt, "")
}
